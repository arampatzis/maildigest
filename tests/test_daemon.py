...
class TestScheduleAll:
    ...
    def test_daily_schedule_uses_wildcard(self):
        ...
        day_of_week_field = next(f for f in trigger_arg.fields if f.name == "day_of_week")
...
class TestRunDaemon:
    def test_exits_on_init_keyring_failure(self):
        ...
        with patch("maildigest.daemon._init_keyring", side_effect=RuntimeError("bad pw")):
    ...
    def test_starts_scheduler_with_loaded_config(self):
        ...
        mock_cfg = MagicMock()
        mock_cfg.mailboxes = []
        with patch("maildigest.daemon._init_keyring"), \
