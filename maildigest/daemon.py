...
from maildigest.config import _init_keyring, load_config
...
def run_daemon(cfg_path: str | None = None) -> None:
    ...
    log.info("Service started. Use 'maildigest service reload' to apply config changes, 'maildigest service stop' to stop.")
    scheduler.start()


def _schedule_all(scheduler: BlockingScheduler, cfg) -> None:
    ...
    for mb in cfg.mailboxes:
        ...
        for hour, minute in mb.schedule_times:
            ...
            log.info(
                ...
                mb.label, hour, minute, days_cron,
            )
